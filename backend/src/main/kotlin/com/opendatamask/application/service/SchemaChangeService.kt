...
import com.opendatamask.domain.port.input.SchemaChangeUseCase
...
class SchemaChangeService(
    ...
    override fun detectChanges(workspaceId: Long): List<SchemaChange> {
        ...
        val newChanges = if (snapshot == null) {
            emptyList() // First snapshot  no changes to detect yet
        } else {
    ...
    override fun getUnresolvedChanges(workspaceId: Long): List<SchemaChange> =
        schemaChangeRepository.findByWorkspaceIdAndStatus(workspaceId, SchemaChangeStatus.UNRESOLVED)

    override fun resolveChange(changeId: Long) {
    ...
    override fun dismissChange(changeId: Long) {
        schemaChangeRepository.findById(changeId).ifPresent { change ->
    ...
    override fun resolveAll(workspaceId: Long) {
